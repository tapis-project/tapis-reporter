...
class Populate:
    def __init__(self):
        self.github_url = "https://raw.githubusercontent.com/tapis-project/tapis-reporting/main"

    def populate_trainings(self):
        trainings_url = requests.get(
            f"{self.github_url}/tapis_trainings.json"
        )
    ...
    def populate_papers(self):
        papers_url = requests.get(
            f"{self.github_url}/tapis_papers.json"
        )
        papers_data = papers_url.json()

        logger.debug(papers_data)

        paper_objects = [Paper(**paper_obj)
                         for paper_obj in papers_data]
